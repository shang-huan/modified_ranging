...
void initFreeQueue(FreeQueue *stack)
{
    ...
    stack->head = 0;
    for (uint8_t i = 0; i < FREE_QUEUE_SIZE; i++)
    {
        stack->freeIndex[i] = i;
    }
}
...
{
    uint8_t index = stack->freeIndex[stack->head];
...
void addRecord(TableLinkedList_t *list, dwTime_t Tx, dwTime_t Rx, int64_t Tf, uint16_t localSeq, uint16_t remoteSeq)
{
    ...
    {
        list->tableBuffer[list->tail].next = index;
        list->tableBuffer[index].pre = list->tail;
        list->tail = index;
    }
}
// 删除最后一条记录
...
{
    table_index_t index = list->head;
    table_index_t notTofIndex = NULL_INDEX;
    while (index != NULL_INDEX)
    {
        // 序号小且Rx、Tx数据完整
        if (list->tableBuffer[index].localSeq < localSeq && list->tableBuffer[index].Rx.full != NULL_TIMESTAMP && list->tableBuffer[index].Tx.full != NULL_TIMESTAMP)
        {
            ...
            if (mode == CLASSIC_MODE)
                return index;
            if (notTofIndex == NULL_INDEX)
            {
                notTofIndex = index;
            }
            // 改进模式下需要Tof数据完整
            if (list->tableBuffer[index].Tf != NULL_INDEX)
                return index;
        }
        index = list->tableBuffer[index].next;
    }
...
bool updateTof(TableLinkedList_t *listA, TableLinkedList_t *listB, table_index_t firstIndex, RangingMODE mode)
{
    ...
    */
    ...
    if (mode == CLASSIC_MODE || listA->tableBuffer[indexA3].Tf == NULL_TF || listB->tableBuffer[indexB2].Tf == NULL_TF)
    {
        ...
        int64_t Tof = ((Ra - Db) * (Rb - Da) + (Ra - Da) * (Rb - Db)) / (2 * (Ra + Db + Rb + Da));
        ...
        DEBUG_PRINT("Tof: %llu,d: %f\n", Tof, Tof * 0.4691763978616);
        return false;
    }
    else
    {
        // Tof数据完整，计算全新方式计算Tof
        int64_t Tof = ((Ra - Db) * (Rb - Da) + (Ra - Da) * (Rb - Db)) - (Rb + Db) * listA->tableBuffer[indexA3].Tf - (Rb + Db + Ra + Da) * listB->tableBuffer[indexB2].Tf;
        Tof = Tof / (Ra + Da);
        listA->tableBuffer[indexA1].Tf = Tof;
